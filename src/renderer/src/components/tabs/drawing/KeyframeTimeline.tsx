...
const KF_LIST = [
  { frame: 6, props: '位置' },
  { frame: 18, props: '位置+旋转', active: true },
  { frame: 33, props: '缩放' },
  { frame: 48, props: '位置' }
]
  ...
  return (
    <div>
      <div style={{ height: '36px', borderBottom: '1px solid var(--ho-border)', position: 'relative', marginBottom: '8px' }}>
        ...
        {[12,30,48].map(f => (
          <div key={f} style={{ position:'absolute',bottom:'10px',left:`${(f/60)*100}%`,width:'5px',height:'5px',backgroundColor:'var(--ho-accent)',transform:'rotate(45deg)' }} />
        ))}
      </div>
      ...
      {KF_LIST.map(kf => (
        <div key={kf.frame} style={{
          display:'flex',justifyContent:'space-between',padding:'2px 6px',
          fontSize:'9px',color:kf.active?'var(--ho-accent)':'var(--ho-text-secondary)',
          backgroundColor:kf.active?'var(--ho-accent-bg)':'transparent',borderRadius:'4px',marginBottom:'2px'
        }}>
          <span>帧 {kf.frame}</span>
          <span>{kf.props}</span>
        </div>
      ))}

      <div style={{ display:'flex',gap:'6px',marginTop:'8px' }}>
        <Button variant="primary" style={{flex:1,height:'22px',fontSize:'9px'}}>添加关键帧</Button>
        <Button variant="ghost" style={{flex:1,height:'22px',fontSize:'9px'}}>删除</Button>
      </div>
    </div>
  )
}
